...
async function getCountries() {
  const data = await fs.readFile(
    path.join(process.cwd(), "src/data/countries_data.json")
  )
...
export default async function CountriesPage() {
  ...
  return (
    <>
      ...
      <div className="m-2">
          {countries.length > 0 ? (
            <DataTable data={countries} columns={countriesTableColumns} toolbar={countriesTableToolbar} toolbarSearchList={countriesTableToolbarSearchList} />
          ) : (<EmptyPlaceholder>
